...
test('SCRAP takes half damage and deals 60% more with regular and charged attacks',()=>{
 for(const charged of [false,true])for(const guarding of [false,true]){
  const tuned=boss(),previous=boss();
  previous.enemies[0].stats={...previous.enemies[0].stats,outgoingDamageMultiplier:1,incomingDamageMultiplier:1};
  tuned.charged=previous.charged=charged;tuned.guarding=previous.guarding=guarding;
  assert.equal(attack(tuned).damage,Math.round(attack(previous).damage*1.6));
 }
 const p=freshProgress();award(p,xpThreshold(26),0);p.upgrade=2;
 const b=createEncounter('argus',p);assert.equal(playerAction(b,'attack',()=>.5).damage,63);
 const exact=boss();exact.heroAttack=120;assert.equal(playerAction(exact,'attack',()=>.5).damage,60);
 const ordinary=createEncounter('heavyDrone',p);assert.equal(playerAction(ordinary,'attack',()=>.5).damage,126);
});
