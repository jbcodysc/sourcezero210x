import {ScrapEscape,showScrapBreach} from './scrap-escape.mjs';
import {securityName,securitySpeaker,securityText} from './security-identity.mjs';
...
const roomFlavor={
 terminal:['Departure board: BELLWETHER / SOLACE. Ask the dispatcher about regional service.','A timetable printed yesterday. Someone has already corrected it by hand.'],
...
export function createFairmontScene(api){
 ...
 return class FairmontScene extends Base {
  ...
  createJunction(){
   ...
   this.interactables.push({...CITY_FIXTURES.departureBoard,id:'bus-return',name:'Check the departure board',kind:'bus-travel',text:p.flags.CH2_COMPLETE?'SOLACE — NEXT REGIONAL DEPARTURE.':'The trail in Fairmont is still unfinished.',range:150});
  ...
  createComplex(){
   ...
   if(m.boss&&!m.isMarket&&p.flags.CH2_ARGUS_WALL_BREACHED)showScrapBreach(this);
  ...
  interact(){
   ...
   if(near.kind==='bus-travel'){this.openRegionalTravel();return;}
   ...
   if(near.kind==='room-door'){
    ...
    if(near.requiresFlag&&!p.flags[near.requiresFlag]){this.say(near.requiresFlag==='CH2_ARGUS_DEFEATED'?securityName(p)+' has locked the exit controls. The service elevator beside the integration chamber will be available once its security control is defeated.':'The door is locked by a local safety interlock. Find this department’s control terminal.');return;}
   ...
   if(near.kind==='puzzle'){if(!p.flags[near.flag]){p.flags[near.flag]=true;p.notes.push(near.text);save();}const lines=[{speaker:'Control terminal',text:near.text}];if(!p.flags.CH2_ARGUS_DEFEATED&&!p.flags.CH2_ARGUS_TERMINAL_WARNING_SEEN){p.flags.CH2_ARGUS_TERMINAL_WARNING_SEEN=true;save();lines.push({speaker:securitySpeaker(p),presentation:'argus',channel:'FACILITY BROADCAST',text:'Work-cell routing updated. Unscheduled access noted.'});}this.showDialogue(story.dialogueLines(lines,p),this.player);return;}
   ...
   if(near.kind==='log'){
    ...
    this.showDialogue((entries||[]).flat().map(text=>text.startsWith('{security}: ')?{speaker:securitySpeaker(p),presentation:'argus',channel:'RECORDED SYSTEM LOG',side:'npc',text:text.slice(12)}:{speaker:m.isMarket?'Staff terminal':'Employee terminal',side:'npc',text:securityText(text,p)}),this.player);return;
   }
  ...
  resumeStory(){
   ...
   else if(pending.type.startsWith('argus-')){const room=FACILITY_ZONES[4]+'-room-5';if(this.location!==room)this.travel(room,mapFor(room).arrival);else if(pending.type==='argus-battle')this.beginBattle('argus');else if(pending.type==='argus-escape')this.runScrapEscape(pending.step);else this.runArgusEncounter();}
  }
  ...
  runScrapEscape(step=0){
   if(this.scrapEscape||!getProgress().flags.CH2_ARGUS_ESCAPE_PENDING)return;
   this.arrival=true;this.locked=true;this.player.walking=false;this.player.dir='up';drawActor(this.player,0);
   this.input.keyboard.resetKeys();resetControls();$('#overlay').innerHTML='';$('#prompt').hidden=true;
   this.scrapEscape=new ScrapEscape(this,{sound,onBreach:()=>this.applyEvent('argus-wall-breached'),onComplete:()=>{
    this.scrapEscape=null;this.applyEvent('argus-escape-complete');this.arrival=false;this.locked=false;
    sound.setMode(explorationMusicMode(this));this.input.keyboard.resetKeys();resetControls();this.resumeStory();
   }});this.scrapEscape.start(step);
  }
  runArgusEncounter(){
  ...
  showChapterEnd(){this.locked=true;this.menu='chapter-end';screenMode('game');$('#overlay').className='menu-ui';$('#overlay').innerHTML='<section class="notebook"><small>CHAPTER 2 COMPLETE</small><h2>The Solace connection.</h2><p>The software rollout leads to Solace. Lou lives there. Return to the regional bus station when you are ready to follow the evidence.</p><button data-action="menu-close">Return to Fairmont</button><button data-action="title">Save and return to title</button></section>';save();}
  openRegionalTravel(){this.locked=true;this.menu='regional-travel';this.writer=null;const complete=getProgress().flags.CH2_COMPLETE;$('#overlay').className='menu-ui';$('#overlay').innerHTML='<section class="notebook service"><small>REGIONAL DEPARTURES</small><h2>Where to?</h2><button data-action="regional-bellwether">Bellwether<small>Return home</small></button>'+(complete?'<button data-action="regional-solace">Solace<small>Follow the rollout records. Meet Lou.</small></button>':'<p>Your investigation in Fairmont is still unfinished.</p>')+'<button data-action="menu-close">Stay in Fairmont</button></section>';resetControls();}
  regionalTravel(destination){if(this.menu!=='regional-travel'||destination==='solace'&&!getProgress().flags.CH2_COMPLETE)return;this.closeMenu();const p=getProgress(),solace=destination==='solace';this.startNarration([solace?'The regional bus pulls away from Fairmont. The rollout records—and Lou’s address—are coming with you.':'The regional bus turns toward Bellwether.','Next stop: '+(solace?'Solace.':'Bellwether.')],()=>{p.location=solace?'solace-transit':'city';p.position=solace?{x:1030,y:1410}:{x:3620,y:4100};save();this.scene.start(solace?'Solace':'Explore',{location:p.location,position:p.position,fromBus:true});},true);}

  handleAction(action){if(action==='regional-bellwether'||action==='regional-solace'){this.regionalTravel(action.slice(9));return;}if(this.coreUI?.action(action))return;if(action==='dialogue-next'&&this.dialog){this.interact();return;}if(this.cutscene||this.arrival)return;if(action==='drink-coffee'){const p=getProgress(),i=p.inventory.indexOf('caramel-macchiato');if(i>=0){p.inventory.splice(i,1);p.hp=Math.min(p.maxHp,p.hp+15);p.speedBoostTurns=CONSUMABLES['caramel-macchiato'].boostTurns;save();}this.openJournal();return;}if(action==='fm-room-key'){if(this.serviceId==='hotel-clerk'){this.closeMenu();this.say('Room 204 is upstairs. Turn the key, settle in, and check your bed when you want to rest.');}return;}if(action.startsWith('fm-')){if(this.menu!=='fairmont-service')return;const kind=action.slice(3);let message;if(kind==='coffee'){if(this.serviceId!=='barista')return;const result=this.applyEvent('buy-coffee');message=result.effects.includes('coffee-bought')?'One caramel macchiato. Best delivered while it is still warm.':'You need '+story.COFFEE_PRICE+' credits.';}else if(['snack','field-meal','resonant-drive','laminate-vest'].includes(kind)){if(!['snack','field-meal'].includes(kind)&&this.serviceId!=='vendor')return;if(this.serviceId==='hotel-clerk')return;message=buy(getProgress(),kind);}else return;save();this.openFairmontService(this.serviceId,message);return;}super.handleAction(action);}
 };
}
