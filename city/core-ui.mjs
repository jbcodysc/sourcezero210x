...
export class CoreGameUI{
 ...
 action(action){if(!action.startsWith('core-'))return false;if(!this.mode)return true;
  ...
  if(action==='core-use'&&item){this.command='use';this.result=useInventoryItem(this.progress(),item.id).message;this.save();this.render();this.overlay.querySelector('[data-action="core-result"]')?.focus();}
  return true;
 }
