...
export function useInventoryItem(s,id){
 ...
 if(item.heal){if(s.hp>=s.maxHp&&!item.speedBoost)return {ok:false,message:'HP is already full.'};const recovered=Math.min(item.heal,s.maxHp-s.hp);s.hp+=recovered;if(item.speedBoost)s.speedBoostTurns=item.boostTurns;if(id==='sandwich')s.snacks--;else s.inventory.splice(s.inventory.indexOf(id),1);return {ok:true,message:'Recovered '+Math.ceil(recovered)+' HP.'+(item.speedBoost?' Speed +30% for the next three combat rounds.':'')};}
