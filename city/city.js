...
class ExploreScene extends SceneBase{
 ...
 update(time,delta){
  ...
  if(this.locked){for(const n of [this.player,...this.npcs])if(n.sheetCharacter){n.walking=false;n.talking=this.dialog?.activeSpeaker===n;drawActor(n,delta);}return;}this.player.talking=false;this.immunity=Math.max(0,this.immunity-delta);const dt=Math.min(delta,45)/1000,k=this.keys;
...
class BattleScene extends SceneBase{
 ...
 create(){
  state.scene=this;this.closed=false;this.rewarded=false;this.battle=createEncounter(state.encounter.id,progress,state.encounter.ids);const b=this.battle;resetControls();$('#scene-status').hidden=true;$('#prompt').hidden=true;$('#overlay').innerHTML='';
 ...
 handleAction(action){
  if(this.closed||this.transition)return;const b=this.battle;if(action==='return'&&['victory','defeat'].includes(b.phase)){this.closed=true;if(b.phase==='defeat'&&state.origin?.scene==='Fairmont'){abortFairmontBattle(progress,state.encounter);restore(progress);progress.snacks=b.snacks;if(state.encounter.spawnId)defeatFairmontSpawn(state.fairmontSlots?.[state.encounter.region],state.encounter.spawnId);const checkpoint=state.encounter.checkpoint;progress.location=checkpoint.location;progress.position=checkpoint.position;save();this.scene.start('Fairmont',checkpoint);return;}if(b.phase==='defeat'){restore(progress);progress.snacks=b.snacks;if(state.encounter.spawnId)defeatedSpawn(state.dungeonSlots[state.encounter.region]||state.slots,state.encounter.spawnId);const inFacility=WATER_FLOORS.includes(state.encounter.region);const tutorial=b.id==='courier'&&!progress.flags.policeReported;progress.location=inFacility?state.encounter.region:tutorial?'lab':'clinic';progress.position=inFacility?{...floorData(state.encounter.region).arrival}:{x:768,y:838};if(tutorial){progress.opening='courier_ready';}save();this.scene.start('Explore',{location:progress.location,position:progress.position});}else this.scene.start(state.origin.scene||'Explore',state.origin);return;}
