...
import {freshProgress,mark,award,buy,restore,xpThreshold,SANDWICH_HEAL,SANDWICH_PRICE,VEST_DEFENSE,equipVest,milestone,storyReward} from './progress.mjs';
...
const locations=new Set(['city','plant-floor','plant-relay',...WATER_FLOORS,...BUILDINGS.map(b=>b.id)]);
...
function remember(scene){if(!scene.player||scene.cutscene||['intro','week_later'].includes(progress.opening))return;progress.location=scene.location;progress.position={x:scene.player.x,y:scene.player.y};save();}
...
function advanceText(scene,delta){if(!scene.writer||!scene.writerNode)return;scene.writer.advance(delta);if(scene.writtenCount!==scene.writer.count){scene.writerNode.textContent=scene.writer.text;sound.letter(scene.writer.characters[scene.writtenCount]||'',scene.voice);scene.writtenCount=scene.writer.count;}}
...
class StartScene extends SceneBase{
 ...
 init(data){this.newSlot=data?.newSlot;this.phase='power';this.slot=null;this.name='Alex';}
 create(){
 ...
 showSlots(){this.phase='slots';sound.setMode('menu');$('#overlay').className='start-ui';$('#overlay').innerHTML='<section class="save-screen"><small>SOURCE ZERO · 210X</small><h2>Choose a file</h2><div class="save-files">'+saveFiles.slots.map((s,i)=>'<button class="save-file" data-action="slot-'+i+'"><b>'+String(i+1).padStart(2,'0')+'</b><span><strong>'+esc(s?s.name:'New game')+'</strong><small>'+(s?'LV '+s.level+' · '+esc(BUILDINGS.find(b=>b.id===s.location)?.name||'Bellwether'):'An ordinary day, still ahead.')+'</small></span><em>'+(s?'CONTINUE':'START')+'</em></button>').join('')+'</div><p class="slot-note">'+(saveAvailable?'Three files. Each keeps its own story.':'Saving is unavailable in this browser. These files will last while this page stays open.')+'</p><button class="back-button" data-action="erase-menu">Erase a file…</button><button class="back-button" data-action="power-back">Back</button></section>';$('#footer-hint').textContent='↑ ↓ choose · Enter select';}
 ...
 launch(s){progress=s;state.activeSlot=this.slot;state.slots=newSpawns();state.dungeonSlots={};state.origin=null;state.encounter=null;if(!locations.has(progress.location))progress.location='lab';save();$('#overlay').innerHTML='';$('#overlay').className='';$('#loading').hidden=false;this.scene.start('Explore',resumeOpening(progress));}
 handleAction(action){if(this.phase==='name-rejected'){if(action==='name-dismiss')this.dismissNameRejection();return;}if(action==='erase-menu'){this.showEraseFiles();return;}if(action==='erase-confirm'&&this.phase==='erase-confirm'){saveAvailable=saveFiles.erase(this.eraseSlot);if(saveAvailable)this.showSlots();else{if(!$('#erase-error'))$('#overlay section').insertAdjacentHTML('beforeend','<p id="erase-error" role="alert">Could not erase this file. Browser storage is unavailable. Your file has been kept.</p>');}return;}if(/^erase-[0-2]$/.test(action)){this.confirmErase(Number(action.slice(6)));return;}if(action==='power'||action==='slots')this.showSlots();else if(action==='power-back')this.showPower();else if(action.startsWith('slot-')){this.slot=Number(action.slice(5));const saved=saveFiles.read(this.slot);if(saved)this.launch(saved);else{this.name='Alex';this.showName();}}else if(action==='name-check')this.showNameCheck();else if(action==='name-edit')this.showName();else if(action==='name-start'&&validateName(this.name).ok)this.launch(freshProgress(this.name));}
}
const ROOM_CONTENT={
...
class ExploreScene extends SceneBase{
 ...
 create(){
  ...
  this.player=actor(this,p.x,p.y,0,110,true);
  ...
  this.onKey=e=>{if(e.repeat)return;const k=e.key.toLowerCase();if(this.cutscene){if(['z','enter',' '].includes(k)){e.preventDefault();this.advanceNarration();}return;}if(this.arrival||this.transitioning)return;if(this.menu||this.dialog?.choiceVisible)chooseKeyboardButton(e);if(['z','enter',' '].includes(k)){e.preventDefault();if(this.menu||this.dialog?.choiceVisible){(document.activeElement?.closest('#overlay button')||$('#overlay button:not(:disabled)'))?.click();}else this.interact();}else if(k==='escape'){e.preventDefault();if(this.dialog)this.closeDialogue();else if(this.menu)this.closeMenu();else this.openJournal();}};
  this.input.keyboard.addCapture([13,32,37,38,39,40,90]);this.input.keyboard.on('keydown',this.onKey);
 ...
 createCity(){
  ...
  for(const b of BUILDINGS){
   ...
   const building=this.add.container(b.x,b.y,[facade]).setDepth(b.y);
  ...
  for(const {x,y,scale}of TREES){
   ...
   this.add.image(x,y,'city-atlas','tree').setOrigin(.5,1).setScale(scale).setDepth(y);
  }
 ...
 update(time,delta){
  ...
  if(this.locked)return;this.immunity=Math.max(0,this.immunity-delta);const dt=Math.min(delta,45)/1000,k=this.keys;
  ...
  if((this.location==='city'||this.floor)&&!(this.floor&&progress.flags.waterRestored)){
   ...
   for(const enemy of this.enemies.values()){
    const slot=slots.find(s=>s.area.id===enemy.id);if(!slot)continue;const zone=slot.area,distance=Math.hypot(enemy.x-this.player.x,enemy.y-this.player.y),chasing=distance<(this.floor?620:480)&&(this.floor||contains(this.player,zone,45));
    ...
    if(!this.immunity&&touchingBounds(enemy.sprite.getBounds(),this.player.sprite.getBounds())){this.beginBattle(enemy.kind,enemy.id);return;}
   }
  }
  const near=this.nearest(),prompt=$('#prompt');if(near){prompt.hidden=false;prompt.innerHTML='<b>Z / ENTER</b>'+esc(near.kind==='npc'?'Talk to '+near.name:near.name);}else prompt.hidden=true;
 }
 ...
 nearest(){return [...this.interactables,...this.npcs.map(n=>({...n,kind:'npc',speaker:n,range:120}))].map(n=>({...n,distance:Math.hypot(n.x-this.player.x,n.y-this.player.y)})).filter(n=>n.distance<=(n.range||122)).sort((a,b)=>a.distance-b.distance)[0];}
 interact(){
 ...
 handleWaterInteraction(near){
  ...
  if(near.kind==='dungeon-rest'){restore(progress);save();this.immunity=2400;say('The first-aid cabinet still works. HP fully restored. Someone has written “PLEASE CONTINUE EXISTING” on the door.');return true;}
  if(near.kind==='vest'){if(progress.flags.vestFound)say('The insulated gear locker is empty. Your vest is in the equipment section of your notebook.');else{mark(progress,'vestFound','Found an insulated work vest near the fourth-floor stairs. It adds '+VEST_DEFENSE+' defense when equipped.');progress.inventory??=[];if(!progress.inventory.includes('insulated-vest'))progress.inventory.push('insulated-vest');progress.armor='insulated-vest';save();say('An insulated work vest, still in its protective wrapper. You put it on. Defense +'+VEST_DEFENSE+'. You can change equipment in your notebook.');}return true;}
  ...
  if(near.kind==='ending'){if(!progress.flags.waterRestored)say('The waterworks overflow blocks the road to Fairmont.');else{mark(progress,'chapterComplete','Bellwether’s water regulator is restored. The Fairmont road is open.');save();this.startNarration(['The water falls back into its channels.','Beyond Bellwether, the road to Fairmont Junction is finally clear.','End of the Bellwether chapter. Your progress is saved.'],()=>{this.cutscene=null;this.locked=true;this.menu='ending';screenMode('game');$('#overlay').className='menu-ui';$('#overlay').innerHTML='<section class="notebook"><small>END OF THIS CHAPTER</small><h2>The road is open.</h2><p>The Cenexis trail continues in Fairmont Junction. For now, Bellwether can breathe again.</p><button data-action="ending-back">Stay in Bellwether</button><button data-action="title">Save and return to title</button></section>';},true);}return true;}
  return false;
 }
 ...
 renderDialogue(){const d=this.dialog,line=d.messages[d.index],hero=line.side==='hero';d.activeSpeaker=hero?this.player:(this.npcs.find(n=>n.id===line.speakerId||n.name===line.speaker||n.id===line.speaker)||d.speaker);d.hero=hero;$('#overlay').className='';$('#overlay').innerHTML='<section class="dialog-box '+(hero?'dialog-hero':'dialog-npc')+'" aria-label="Conversation"><div class="dialog-speaker">'+esc(line.speaker)+'</div><p class="typed-copy">'+textSlot(line.text)+'</p><button data-action="dialogue-next">Continue <span>Z / ENTER ▾</span></button></section>';beginText(this,line.text,d.activeSpeaker?.row||0);this.positionDialogue();}
 positionDialogue(){const d=this.dialog,box=$('#overlay .dialog-box');if(!d||!box)return;const camera=this.cameras.main,who=d.choiceVisible?this.player:d.activeSpeaker||d.speaker,point=conversationAnchor({x:who.x-camera.scrollX,y:who.y-camera.scrollY},VIEW,{width:$('#stage').clientWidth,height:$('#stage').clientHeight},{width:box.offsetWidth,height:box.offsetHeight},d.choiceVisible||d.hero);box.style.left=point.x+'%';box.style.top=point.y+'%';}
 ...
 closeDialogue(){this.dialog=null;this.writer=null;this.locked=this.arrival||!!this.cutscene||this.transitioning;$('#overlay').innerHTML='';$('#overlay').className='';resetControls();this.input.keyboard.resetKeys();this.immunity=Math.max(this.immunity,900);}
 ...
 openJournal(){if(this.transitioning||this.dialog||this.arrival||this.cutscene)return;remember(this);this.locked=true;this.menu='journal';this.writer=null;$('#prompt').hidden=true;$('#overlay').className='menu-ui';$('#overlay').innerHTML='<section class="notebook"><small>'+esc(heroName()+"’s notebook").toUpperCase()+'</small><h2>'+ (progress.flags.waterRestored?'A road beyond Bellwether':progress.flags.factoryDone?'The water beneath Bellwether':'Things that do not add up')+'</h2><div class="notebook-stats"><span>LV '+progress.level+'</span><span>HP '+Math.ceil(progress.hp)+' / '+progress.maxHp+'</span><span>'+progress.credits+' credits</span><span>'+progress.snacks+' sandwiches</span></div><p class="notebook-meta">'+progress.xp+' XP · next level at '+xpThreshold(progress.level+1)+' · '+progress.visited.filter(x=>BUILDINGS.some(b=>b.id===x)).length+' / '+BUILDINGS.length+' buildings visited</p><div class="equipment-slot"><div><small>BODY</small><strong>'+ (progress.armor==='insulated-vest'?'Insulated work vest · +'+VEST_DEFENSE+' defense':'Empty')+'</strong></div>'+(progress.inventory?.includes('insulated-vest')?'<button data-action="equip-vest">'+(progress.armor==='insulated-vest'?'Unequip':'Equip vest')+'</button>':'<span>No defensive gear found</span>')+'</div><div class="notes">'+(progress.notes.length?progress.notes.map(n=>'<p>'+esc(n)+'</p>').join(''):'<p>'+esc(progress.opening==='await_mira'?'Mira is arranging the morning shipment. I should check in.':'The courier is here for a delivery. An ordinary day, apparently.')+'</p>')+'</div><div class="menu-row"><button data-action="eat">Eat sandwich</button><button data-action="menu-close">Return <span>ESC</span></button></div><p class="save-note">'+(saveAvailable?'File '+(state.activeSlot+1)+' · Saved on this browser.':'Browser storage is unavailable. Keep this tab open to retain progress.')+'</p><button class="quiet" data-action="new-game">Start a new game in this file…</button><button class="quiet" data-action="title">Save and return to title</button></section>';resetControls();this.input.keyboard.resetKeys();}
 closeMenu(){this.menu=null;this.locked=false;$('#overlay').innerHTML='';$('#overlay').className='';resetControls();this.input.keyboard.resetKeys();this.immunity=Math.max(this.immunity,1500);}
 handleAction(action){
  ...
  if(action==='equip-vest'){equipVest(progress);save();this.openJournal();return;}
...
class BattleScene extends SceneBase{
 ...
 create(){
  state.scene=this;this.closed=false;this.rewarded=false;this.battle=createEncounter(state.encounter.id,progress);const b=this.battle;resetControls();$('#scene-status').hidden=true;$('#prompt').hidden=true;$('#overlay').innerHTML='';
  ...
  this.time.delayedCall(BATTLE_START_MS,()=>{if(this.closed)return;this.transition?.destroy();this.transition=null;screenMode('battle');this.renderHUD();sound.setMode('battle');this.input.keyboard.resetKeys();});
  this.onKey=e=>{if(e.repeat||this.transition)return;chooseKeyboardButton(e);if(['z','enter',' '].includes(e.key.toLowerCase())){e.preventDefault();if(finishText(this))return;(document.activeElement?.closest('#overlay button:not(:disabled)')||$('#overlay button:not(:disabled)'))?.click();}};
  this.input.keyboard.addCapture([13,32,37,38,39,40,90]);this.input.keyboard.on('keydown',this.onKey);this.events.once('shutdown',()=>{this.closed=true;this.writer=null;this.transition?.destroy();this.transition=null;sound.battle.stop();sound.stopEffects('fanfare');sound.stopEffects('victory');this.input.keyboard.off('keydown',this.onKey);resetControls();});
 }
 update(time,delta){if(this.closed)return;if(time-this.patternTime>=32){drawBattleBackdrop(this.pattern.context,time);this.pattern.refresh();this.patternTime=time;}if(this.transition){this.transitionElapsed+=delta;this.transition.draw(this.transitionElapsed);return;}advanceText(this,delta);const before=this.battle.phase;rollHealth(this.battle,delta/1000);this.updateVitals();if(before!==this.battle.phase&&this.battle.phase==='defeat')this.renderHUD();}
 reward(){if(this.rewarded)return;this.rewarded=true;sound.setMode('victory');sound.effect('victory');const b=this.battle;progress.hp=Math.max(1,Math.ceil(b.hp));progress.snacks=b.snacks;const rewards=encounterRewards(b);this.levels=award(progress,rewards.xp,rewards.credits);if(b.id==='courier'){mark(progress,'courierDone');progress.opening='aftermath';}if(b.id==='bastion')mark(progress,'bastionDefeated');if(b.id==='regulator')mark(progress,'waterRestored','The regulator is free of the Cenexis override. The overflow has stopped, and the south road can drain.');if(state.encounter.spawnId)defeatedSpawn(state.dungeonSlots[state.encounter.region]||state.slots,state.encounter.spawnId);save();}
 renderHUD(){
  const b=this.battle,finished=['victory','defeat'].includes(b.phase),foe=targetEnemy(b),rewards=encounterRewards(b);this.renderEnemies();if(b.phase==='victory')this.reward();if(finished)sound.battle.stop();$('#overlay').className='battle-ui'+(finished?' battle-finished':'')+(livingEnemies(b).length>1?' battle-multiple':'');
 ...
 handleAction(action){
  if(this.closed||this.transition)return;const b=this.battle;if(action==='return'&&['victory','defeat'].includes(b.phase)){this.closed=true;if(b.phase==='defeat'){restore(progress);progress.snacks=b.snacks;if(state.encounter.spawnId)defeatedSpawn(state.dungeonSlots[state.encounter.region]||state.slots,state.encounter.spawnId);const inFacility=WATER_FLOORS.includes(state.encounter.region);const tutorial=b.id==='courier'&&!progress.flags.policeReported;progress.location=inFacility?state.encounter.region:tutorial?'lab':'clinic';progress.position=inFacility?{...floorData(state.encounter.region).arrival}:{x:768,y:838};if(tutorial){progress.opening='courier_ready';}save();this.scene.start('Explore',{location:progress.location,position:progress.position});}else this.scene.start('Explore',state.origin);return;}
  ...
  this.renderHUD();if(b.phase==='victory')return;
  this.time.delayedCall(Math.max(950,this.writer.duration+200),()=>this.resolveEnemyTurn());
 }
}
if(!P)$('#loading').innerHTML='<p>The game engine could not load. Please reload the page.</p>';
 ...
 window.addEventListener('pageshow',e=>{if(!e.persisted)return;sound.setVisible(!document.hidden);const s=state.scene;if(s instanceof StartScene)sound.setMode(s.phase==='power'?'title':'menu');else if(s instanceof ExploreScene)sound.setMode(s.floor||s.location==='water'?'dungeon':s.location==='city'?'city':s.location==='lab'?'lab':'interior');else if(s instanceof BattleScene&&!['victory','defeat'].includes(s.battle.phase))sound.setMode(s.transition?'transition':'battle');});
 new P.Game({type:P.AUTO,parent:'phaser',width:VIEW.width,height:VIEW.height,backgroundColor:'#101d29',pixelArt:true,roundPixels:true,antialias:false,scale:{mode:P.Scale.FIT,autoCenter:P.Scale.CENTER_BOTH},scene:[StartScene,ExploreScene,BattleScene],input:{keyboard:true,touch:true},render:{powerPreference:'low-power'},audio:{noAudio:true}});
}
