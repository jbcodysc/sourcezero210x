...
class BattleScene extends SceneBase{
 ...
 renderHUD(){
  const b=this.battle,finished=['victory','defeat'].includes(b.phase),foe=targetEnemy(b),rewards=encounterRewards(b);this.renderEnemies();if(b.phase==='victory')this.reward();if(b.phase==='defeat')sound.setMode('defeat');$('#overlay').className='battle-ui'+(finished?' battle-finished':'')+(livingEnemies(b).length>1?' battle-multiple':'');
