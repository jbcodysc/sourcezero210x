...
import {ShopUI} from './shop-ui.mjs';
import {hasMagic,inventoryEntries} from './core-status.mjs';
import {preloadScreenAttacks,registerScreenAttacks,TidalWaveEffect,MissileVolleyEffect} from './screen-attacks.mjs';
import {MISSILE_TAUNT,SCRAP_FAREWELL,SCRAP_NAME} from '../fairmont/security-identity.mjs';
...
import {CHAPTERS as BASE_CHAPTERS,createChapterProgress} from './chapter-start.mjs';
import {ALL_LOCATIONS as SOLACE_LOCATIONS} from '../solace/world.mjs';
...
import {isArgusLine,argusPanelMarkup,argusTextMarkup,argusTypingStyle} from './argus-dialogue.mjs';
...
import {createEncounter,applyEnemyImpact,finishEnemyAnimation,revealReinforcement,effectiveSpeed,rollHealth,ENEMIES,livingEnemies,commandMember,targetEnemy,selectTarget,encounterRewards} from './encounters.mjs';
import {awardParty,restoreParty} from './party.mjs';
...
const CHAPTERS=[...BASE_CHAPTERS,...(developerCheckpointsEnabled(globalThis.location?.search||'')?DEV_CHAPTERS:[])];
const locations=new Set([...SOLACE_LOCATIONS,...FAIRMONT_LOCATIONS,'city','plant-floor','plant-relay',...WATER_FLOORS,...BUILDINGS.map(b=>b.id)]);
...
function chooseKeyboardButton(e){if(document.activeElement?.matches('input,textarea'))return;const key=e.key.toLowerCase();if(!['arrowup','arrowdown','arrowleft','arrowright','w','a','s','d'].includes(key))return;const buttons=[...document.querySelectorAll('#overlay button:not(:disabled)')];if(!buttons.length)return;e.preventDefault();let i=buttons.indexOf(document.activeElement);if(i<0)i=buttons.findIndex(b=>b.classList.contains('command-selected'));i=(i+(['arrowleft','arrowup','w','a'].includes(key)?-1:1)+buttons.length)%buttons.length;buttons[i]?.focus();}
class StartScene extends SceneBase{
 ...
 launch(s){progress=s;state.activeSlot=this.slot;state.slots=newSpawns();state.dungeonSlots={};state.fairmontSlots={};state.solaceSlots={};state.origin=null;state.encounter=null;state.encounterImmunityUntil=0;if(!locations.has(progress.location))progress.location='lab';save();$('#overlay').innerHTML='';$('#overlay').className='';$('#loading').hidden=false;this.scene.start(progress.location.startsWith('solace-')?'Solace':progress.location.startsWith('fairmont')?'Fairmont':'Explore',resumeOpening(progress));}
 handleAction(action){if(action==='chapter-menu'){this.showChapters();return;}if(CHAPTERS.some(c=>'chapter-'+c.id===action)){this.chapter=CHAPTERS.find(c=>'chapter-'+c.id===action).id;this.showChapterSlots();return;}if(action==='chapter-files'){this.showChapterSlots();return;}if(/^chapter-slot-[0-2]$/.test(action)){this.confirmChapterSlot(Number(action.slice(13)));return;}if(action==='chapter-replace'&&this.phase==='chapter-overwrite'){this.showName();return;}if(this.phase==='name-rejected'){if(action==='name-dismiss')this.dismissNameRejection();return;}if(action==='erase-menu'){this.showEraseFiles();return;}if(action==='erase-confirm'&&this.phase==='erase-confirm'){saveAvailable=saveFiles.erase(this.eraseSlot);if(saveAvailable)this.showSlots();else{if(!$('#erase-error'))$('#overlay section').insertAdjacentHTML('beforeend','<p id="erase-error" role="alert">Could not erase this file. Browser storage is unavailable. Your file has been kept.</p>');}return;}if(/^erase-[0-2]$/.test(action)){this.confirmErase(Number(action.slice(6)));return;}if(action==='power'||action==='slots')this.showSlots();else if(action==='power-back')this.showPower();else if(action.startsWith('slot-')){this.slot=Number(action.slice(5));const saved=saveFiles.read(this.slot);if(saved)this.launch(saved);else{this.chapter=1;this.name='Alex';this.showName();}}else if(action==='name-check')this.showNameCheck();else if(action==='name-edit')this.showName();else if(action==='name-start'&&validateName(this.name).ok)this.launch(String(this.chapter).startsWith('3-')?createSolaceCheckpoint(this.chapter,this.name):createChapterProgress(this.chapter,this.name));}
}
const ROOM_CONTENT={
...
class ExploreScene extends SceneBase{
 ...
 create(){
  ...
  document.title='SOURCE ZERO · Bellwether';$('#stage').setAttribute('aria-label','Explore '+(building?.name||this.floor?.title||'Bellwether'));
 ...
 renderDialogue(){const d=this.dialog,line=d.messages[d.index],hero=line.side==='hero';d.activeSpeaker=hero?this.player:(this.npcs.find(n=>n.id===line.speakerId||n.name===line.speaker||n.id===line.speaker)||d.speaker);d.hero=hero;d.argus=isArgusLine(line);$('#overlay').className='';$('#overlay').innerHTML=d.argus?argusPanelMarkup(line):'<section class="dialog-box '+(hero?'dialog-hero':'dialog-npc')+(this.bellwetherRunning?' dialog-bellwether':'')+'" aria-label="Conversation"><div class="dialog-speaker">'+esc(line.speaker)+':</div><p class="typed-copy">'+textSlot(line.text)+'</p><button data-action="dialogue-next" aria-label="Continue dialogue">▾</button></section>';beginText(this,d.argus?line.text.toUpperCase():line.text,d.argus?'argus':d.activeSpeaker?.row||0,d.argus?argusTypingStyle(line):null);this.positionDialogue();}
 ...
 openService(id,message=''){this.locked=true;this.menu=id;this.writer=null;const offerings=id==='pia'?[['hotel','Rest · 14 credits','Restore all HP']]:id==='beck'?[['upgrade','Insulated grip · 85 credits',progress.upgrade?'Already equipped':'+6 attack']]:[['snack','Pocket sandwich · '+SANDWICH_PRICE+' credits','Restore '+SANDWICH_HEAL+' HP']];if(id!=='pia'){this.openShop({title:id==='beck'?'Beck’s workbench':id==='marty'?'Harvest Diner':'Harrow General Store',region:'BELLWETHER',offers:offerings,purchase:kind=>buy(progress,kind)});return;}$('#overlay').className='menu-ui';$('#overlay').innerHTML='<section class="notebook service"><small>BELLWETHER · '+progress.credits+' CREDITS</small><h2>'+esc(id==='pia'?'Juniper Hotel':id==='beck'?'Beck’s workbench':id==='marty'?'Harvest Diner':'Harrow General Store')+'</h2><p>'+esc(message||'Take a moment before heading out.')+'</p>'+offerings.map(([key,label,sub])=>'<button data-action="buy-'+key+'">'+esc(label)+'<small>'+esc(sub)+'</small></button>').join('')+'<button class="quiet" data-action="menu-close">Back to town life <span>ESC</span></button></section>';resetControls();this.input.keyboard.resetKeys();}
 openShop(options){this.shopUI??=new ShopUI(this,{progress:()=>progress,save,overlay:$('#overlay'),reset:resetControls});this.shopUI.open(options);}
 installCoreUI(){
 ...
 closeMenu(){this.shopUI?.clear();if(this.coreUI?.mode){this.coreUI.close();return;}this.menu=null;this.locked=!!(this.dialog||this.cutscene||this.arrival||this.transitioning);$('#overlay').innerHTML='';$('#overlay').className='';resetControls();this.input.keyboard.resetKeys();}

 handleAction(action){
  if(this.shopUI?.action(action))return;
...
const SolaceScene=createSolaceScene({Base:ExploreScene,getProgress:()=>progress,state,save,remember,control,resetControls,screenMode,sceneUI,advanceText,finishText,chooseKeyboardButton,VIEW,sound,actor,drawActor,prepareArt,prepareWaterArt});
class BattleScene extends SceneBase{
 constructor(){super('Battle');}
 preload(){preloadScreenAttacks(this,state.encounter?.id==='argus');}
 init(data){this.transitionSource=data?.transitionSource||null;this.transition=null;this.transitionElapsed=0;}
 create(){
  state.scene=this;registerScreenAttacks(this);this.enemyEffect=null;this.bossSpeech=null;this.closed=false;this.rewarded=false;this.notice=null;this.goodsOpen=false;this.commandSelection=new BattleSelection($('#overlay'));this.actionPresentation=null;this.battle=createEncounter(state.encounter.id,progress,state.encounter.ids);this.battle.storyNoRun=!!state.encounter.noRun;const b=this.battle;resetControls();$('#scene-status').hidden=true;$('#prompt').hidden=true;$('#overlay').innerHTML='';
 ...
 update(time,delta){
  ...
  if(this.transition){this.transitionElapsed+=delta;this.transition.draw(this.transitionElapsed);return;}
  if(this.bossSpeech){advanceText(this,delta);return;}
  // Recovery visibly counts upward even while its acknowledgement owns input.
  // Falling HP remains paused during notices, as in the existing battle UI.
  if(this.notice){rollHealth(this.battle,delta/1000,{healingOnly:true});this.updateVitals();return;}
  const before=this.battle.phase,beforeMember=this.battle.commandIndex;rollHealth(this.battle,delta/1000);commandMember(this.battle);this.updateVitals();
  if(this.battle.phase==='command'&&beforeMember!==this.battle.commandIndex){this.renderHUD();finishText(this);}
  ...
  if(this.actionPresentation&&this.writer?.done){this.actionPresentation.remaining-=Math.min(delta,100);if(this.actionPresentation.remaining<=0){this.actionPresentation=null;if(this.battle.phase==='reinforcement'){revealReinforcement(this.battle);this.actionPresentation={remaining:1100};this.renderHUD();return;}if(this.battle.phase==='bossRetreat'){this.showBossSpeech(SCRAP_FAREWELL,()=>{this.battle.phase='victory';this.battle.message='S.C.R.A.P. disengages. You held your ground.';this.renderHUD();});return;}if(['victory','escaped'].includes(this.battle.phase)){this.renderHUD();return;}this.resolveTurn();}}
 }
 reward(){if(this.rewarded)return;this.rewarded=true;sound.setMode('victory');sound.effect('victory');const b=this.battle;progress.hp=Math.max(1,Math.ceil(b.hp));carryBattleInventory(progress,b);const rewards=encounterRewards(b);const beforeStats=playerStats(progress);this.levels=award(progress,rewards.xp,rewards.credits);this.levelNotices=awardParty(progress,rewards.xp);if(this.levels)this.notice={kind:'level',page:0,before:beforeStats,after:playerStats(progress),level:progress.level,name:heroName()};else this.notice=this.levelNotices.shift()||null;if(b.id==='courier'){mark(progress,'courierDone');progress.opening='aftermath';}if(b.id==='bastion')mark(progress,'bastionDefeated');if(b.id==='regulator')mark(progress,'waterRestored','The regulator is free of the Cenexis override. The overflow has stopped, and the south road can drain.');if(state.origin?.scene==='Solace'){state.origin.onBattleWon?.(progress,state.encounter);}else if(state.origin?.scene==='Fairmont'){finishFairmontBattle(progress,state.encounter);if(state.encounter.spawnId)defeatFairmontSpawn(state.fairmontSlots?.[state.encounter.region],state.encounter.spawnId);}else if(state.encounter.spawnId)defeatedSpawn(state.dungeonSlots[state.encounter.region]||state.slots,state.encounter.spawnId);save();}
 renderHUD(){
  const b=this.battle,member=commandMember(b),finished=['victory','defeat','escaped'].includes(b.phase)&&!this.actionPresentation,foe=targetEnemy(b),rewards=encounterRewards(b);this.renderEnemies();if(b.phase==='victory'&&!this.actionPresentation)this.reward();if(b.phase==='defeat')sound.setMode('defeat');if(b.phase==='escaped')sound.setMode('escaped');$('#overlay').className='battle-ui'+(finished?' battle-finished':'')+(livingEnemies(b).length>1?' battle-multiple':'')+(b.party.length>1?' battle-party':'');
  ...
  const resultTitle=b.phase==='escaped'?'Got away.':b.phase==='victory'?(b.id==='argus'?'S.C.R.A.P. withdraws.':b.id==='regulator'?'Pressure restored.':b.enemy.kind==='human'?'They back down.':b.enemy.kind==='animal'?'They scurry away.':'Signal interrupted.'):'Still here.';
  const resultDetail=b.phase==='escaped'?'No XP or credits earned. You have two seconds to get clear.':b.phase==='victory'?'+'+rewards.xp+' XP · '+rewards.credits+' credits':b.id==='courier'?'Take a breath. You can try the pickup again.':WATER_FLOORS.includes(state.encounter.region)?'The emergency system brings you back to this floor’s stair lobby. Your discoveries and equipment are safe.':'The clinic can get you back on your feet. Your notes are safe.';
  const goods=inventoryEntries(b).filter(item=>item.heal&&item.count>0),disabled=b.phase!=='command';
  const commands=this.goodsOpen?goods.map(item=>button(esc(item.name),'item:'+item.id,'× '+item.count+' · Restore '+item.heal+' HP'+(item.speedBoost?' · Speed +30% / 3 turns':''),disabled)).join('')+button('Back','goods-back','Choose another command',disabled):button('Attack','attack',member.id==='hero'?'Vibrosword'+(progress.upgrade>=2?' · resonant edge':progress.upgrade?' · insulated grip':''):(member.equipment?.weapon||member.weapon||'Weapon'),disabled)+button('Guard','guard','Protect this turn',disabled)+button('Goods','goods',goods.reduce((n,item)=>n+item.count,0)+' healing items',disabled||!goods.length)+button('Run','run','',disabled||!canRun(b));
  const panel=finished?'<section class="result"><small>'+(b.phase==='victory'?'ENCOUNTER COMPLETE':b.phase==='escaped'?'ESCAPED':'A MOMENT TO RECOVER')+'</small><h2>'+resultTitle+'</h2><p>'+resultDetail+'</p><button data-action="return">Continue →</button></section>':'<section class="battle-panel"><div class="battle-panel-head"><span>'+esc(member.name).toUpperCase()+'</span><small>'+(this.goodsOpen?'GOODS':disabled?'TURN RESOLVING':b.party.length>1?'CHOOSE ACTION · '+(b.commandIndex+1)+' / '+b.party.length:'YOUR MOVE')+'</small></div>'+this.healTargetButtons()+'<div class="battle-command-grid">'+commands+'</div></section>';
  $('#overlay').innerHTML='<div class="battle-topline"><div><span>UNSCHEDULED ENCOUNTER</span><h1>'+esc(b.enemy.name)+'</h1></div><small>TURN '+b.turn+'</small></div><div class="battle-log" role="status"><div class="typed-copy">'+textSlot(b.message)+'</div></div>'+panel+this.targetButtons()+'<div class="enemy-status"><span>'+esc(foe.stats.name.toUpperCase())+'</span><div><i id="enemy-hp"></i></div><small>'+(b.charged?(foe.stats.kind==='human'?'WINDING UP':'CAPACITORS CHARGING'):'HOSTILE INTENT')+'</small></div>'+this.partyVitals();
  if(finished)this.writer=null;else beginText(this,b.message);this.updateVitals();fitBattleText($('#overlay'));if(this.notice)this.renderNotice();else this.commandSelection?.restore(this.goodsOpen,!disabled&&!finished);
 }
 renderNotice(){
  this.writer=null;
  ...
  const detail=stats?'<dl class="level-gains">'+[['health','Maximum HP'],['attack','Attack'],['defense','Defense'],['speed','Speed']].map(([key,label])=>'<div><dt>'+label+'</dt><dd><strong>+'+(n.after[key]-n.before[key])+'</strong><span>'+n.before[key]+' → '+n.after[key]+'</span></dd></div>').join('')+'</dl>':'';
  $('#overlay').insertAdjacentHTML('beforeend','<div class="battle-notice-shade"><section class="battle-notice '+(level?'level-notice':'heal-notice')+'" role="dialog" aria-modal="true" aria-label="'+(level?'Level up':'Healing confirmation')+'"><small>'+(level?'LEVEL UP':esc(n.itemName||'HEALING ITEM').toUpperCase())+'</small><h2 class="'+(level&&!stats?'rainbow-level':'')+'">'+esc(title)+'</h2>'+(n.speedBoost?'<p>Speed +30% for three turns.</p>':'')+detail+'<button data-action="notice-next">'+(stats?'Continue →':'Continue')+' <span>ENTER</span></button></section></div>');
 ...
 advanceNotice(){
  ...
  this.notice=null;
  if(n.kind==='level'){if(this.levelNotices?.length){this.notice=this.levelNotices.shift();$('#overlay .battle-notice-shade').remove();this.renderNotice();}else this.handleAction('return');return;}
 ...
 resolveTurn(){
  ...
  if(result.type==='missile-volley'){this.showBossSpeech([MISSILE_TAUNT],()=>this.playScreenAttack(result));return;}
  ...
  this.actionPresentation={remaining:result.actor==='hero'?1800:950};
  if(result.itemId&&result.ok)this.notice={kind:'healing',recovered:result.recovered,itemName:result.itemName,speedBoost:result.speedBoost};
  this.renderHUD();
 }
 playScreenAttack(result){
  const b=this.battle;this.actionPresentation=null;this.renderHUD();this.writer=null;
  const Effect=result.type==='missile-volley'?MissileVolleyEffect:TidalWaveEffect;
  this.enemyEffect=new Effect(this,{host:$('#stage'),sound,
   onImpact:index=>{if(!this.closed)applyEnemyImpact(b,result.impact,index);},
   onComplete:()=>{
 ...
 showBossSpeech(pages,done){
  this.actionPresentation=null;this.bossSpeech={pages,index:0,done};this.renderBossSpeech();
 }
 renderBossSpeech(){
  this.renderHUD();const text=this.bossSpeech.pages[this.bossSpeech.index];
  $('#overlay').insertAdjacentHTML('beforeend',argusPanelMarkup({speaker:SCRAP_NAME,text}).replace('data-action="dialogue-next"','data-action="boss-speech-next"'));
  $('#overlay').classList.add('battle-boss-speaking');
  beginText(this,text.toUpperCase(),'argus');this.writerNode=$('#overlay .dialog-argus [data-typing]');
  this.input.keyboard.resetKeys();resetControls();
 }
 advanceBossSpeech(){
  if(!this.bossSpeech||finishText(this))return;
  const speech=this.bossSpeech;if(++speech.index<speech.pages.length){this.renderBossSpeech();return;}
  this.bossSpeech=null;this.writer=null;$('#overlay').classList.remove('battle-boss-speaking');speech.done();
 }
 healTargetButtons(){const b=this.battle;if(!this.goodsOpen||b.party.length<2)return '';return '<div class="battle-heal-targets" aria-label="Choose item recipient">'+b.party.map(member=>'<button data-action="heal-target:'+member.id+'" aria-pressed="'+(b.healTargetId===member.id)+'">'+esc(member.name)+' · '+Math.ceil(member.hp)+' / '+member.maxHp+' HP</button>').join('')+'</div>';}
 partyVitals(){const b=this.battle;return '<div class="battle-party-vitals">'+b.party.map((member,index)=>'<section class="hero-vitals '+(index===b.commandIndex&&b.phase==='command'?'member-selected':'')+(member.hp<=0?' member-down':'')+'"><div class="vital-title"><b>'+esc(member.name).toUpperCase()+'</b><small>LV '+String(member.level).padStart(2,'0')+' · SPD '+Math.round(effectiveSpeed(member)*10)/10+(member.speedBoostTurns>0?' ↑ '+member.speedBoostTurns+'T':'')+'</small></div><div class="hp-number"><span>HP</span><strong id="'+member.id+'-hp"></strong><small>/ '+member.maxHp+'</small></div><div class="vital-track"><i id="'+member.id+'-hp-bar"></i></div><div class="vital-bottom">'+esc(member.equipment?.weapon||member.weapon)+(hasMagic(member)?'<span>MP '+(member.mp??0)+' / '+(member.maxMp??0)+'</span>':'')+(member.slowTurns>0?'<span>FOAM · '+member.slowTurns+'T</span>':'')+'</div></section>').join('')+'</div>';}
 updateVitals(){const b=this.battle;for(const member of b.party){if($('#'+member.id+'-hp'))$('#'+member.id+'-hp').textContent=String(Math.ceil(member.hp)).padStart(3,'0');if($('#'+member.id+'-hp-bar'))$('#'+member.id+'-hp-bar').style.width=member.hp/member.maxHp*100+'%';}if($('#enemy-hp'))$('#enemy-hp').style.width=b.enemyHp/b.enemyMaxHp*100+'%';}
 handleAction(action){
  if(this.closed||this.transition)return;if(this.bossSpeech){if(action==='boss-speech-next')this.advanceBossSpeech();return;}if(this.battle.phase==='enemyAnimation')return;if(this.notice){if(action==='notice-next')this.advanceNotice();return;}if(this.actionPresentation)return;const b=this.battle;if(action==='return'&&['victory','defeat','escaped'].includes(b.phase)){this.closed=true;if(b.phase==='escaped'){escapeProgress(progress,b);save();this.scene.start(state.origin.scene||'Explore',{...state.origin,afterBattle:true});return;}if(b.phase==='defeat'&&state.origin?.scene==='Solace'){state.origin.onBattleLost?.(progress,state.encounter);carryBattleInventory(progress,b);restore(progress);restoreParty(progress);const checkpoint={location:'solace-clinic',position:{x:768,y:760}};progress.location=checkpoint.location;progress.position=checkpoint.position;save();this.scene.start('Solace',{...checkpoint,afterBattle:true});return;}if(b.phase==='defeat'&&state.origin?.scene==='Fairmont'){abortFairmontBattle(progress,state.encounter);restore(progress);carryBattleInventory(progress,b);restoreParty(progress);if(state.encounter.spawnId)defeatFairmontSpawn(state.fairmontSlots?.[state.encounter.region],state.encounter.spawnId);const checkpoint={location:'fairmont-clinic',position:{x:768,y:760}};progress.location=checkpoint.location;progress.position=checkpoint.position;save();this.scene.start('Fairmont',{...checkpoint,afterBattle:true});return;}if(b.phase==='defeat'){restore(progress);carryBattleInventory(progress,b);restoreParty(progress);if(state.encounter.spawnId)defeatedSpawn(state.dungeonSlots[state.encounter.region]||state.slots,state.encounter.spawnId);const inFacility=WATER_FLOORS.includes(state.encounter.region);const tutorial=b.id==='courier'&&!progress.flags.policeReported;progress.location=inFacility?state.encounter.region:tutorial?'lab':'clinic';progress.position=inFacility?{...floorData(state.encounter.region).arrival}:{x:768,y:838};if(tutorial){progress.opening='courier_ready';}save();this.scene.start('Explore',{location:progress.location,position:progress.position,afterBattle:true});}else this.scene.start(state.origin.scene||'Explore',{...state.origin,afterBattle:true});return;}
  if(action.startsWith('heal-target:')&&b.phase==='command'){const id=action.slice(12);if(b.party.some(member=>member.id===id)){b.healTargetId=id;this.renderHUD();finishText(this);}return;}
  ...
  if(finishText(this))return;
  const result=beginRound(b,action);if(!result.ok){if(result.message){b.message=result.message;this.renderHUD();}return;}
  this.goodsOpen=false;if(result.awaitingCommands){this.renderHUD();finishText(this);this.input.keyboard.resetKeys();resetControls();return;}this.resolveTurn();
 }
}
if(!P)$('#loading').innerHTML='<p>The game engine could not load. Please reload the page.</p>';
 ...
 $('#touch-act').onclick=()=>{const s=state.scene;if(s instanceof ExploreScene&&!s.menu&&!s.dialog?.choiceVisible)s.interact();else ($('#overlay .command-selected:not(:disabled)')||$('#overlay button:not(:disabled)'))?.click();};
 ...
 new P.Game({type:P.AUTO,parent:'phaser',width:VIEW.width,height:VIEW.height,backgroundColor:'#101d29',pixelArt:true,roundPixels:true,antialias:false,scale:{mode:P.Scale.FIT,autoCenter:P.Scale.CENTER_BOTH},scene:[StartScene,ExploreScene,FairmontScene,SolaceScene,BattleScene],input:{keyboard:true,touch:true},render:{powerPreference:'low-power'},audio:{noAudio:true}});
}
