...
export function rollHealth(state,deltaSeconds){if(!Number.isFinite(deltaSeconds)||deltaSeconds<=0||['victory','escaped','defeat'].includes(state.phase))return;const dt=Math.min(deltaSeconds,.1);state.hp=state.hp>state.targetHp?Math.max(state.targetHp,state.hp-dt*HP_DRAIN_PER_SECOND):Math.min(state.targetHp,state.hp+dt*HP_RECOVERY_PER_SECOND);if(state.hp<=0){state.phase='defeat';state.message='Your knees give way. There is still time to try again.';}}
