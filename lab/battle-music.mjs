export class BattleMusic {
 constructor(url,{contextFactory=()=>new (globalThis.AudioContext||globalThis.webkitAudioContext)(),fetcher=(...args)=>globalThis.fetch(...args),onError=()=>{},onReady=()=>{},resumeOnStart=false,initialVolume=.55}={}){
  ...
  this.resumeOnStart=resumeOnStart;this.initialVolume=initialVolume;this.offset=0;this.startedAt=0;
 }
 ...
 sync(){
  ...
  source.buffer=this.buffer;source.loop=true;gain.gain.value=this.initialVolume;source.connect(gain);gain.connect(this.context.destination);
